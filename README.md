...
Update `docker/prometheus/prometheus.yml` to scrape over HTTPS with the CA certificate:
...
Copy `certs/ca.crt` to the Prometheus host and mount it into the container (or place it at the path referenced above).
